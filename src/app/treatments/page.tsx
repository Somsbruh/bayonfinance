        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            <div className="space-y-6">
                {loading ? (
                    <div className="grid grid-cols-1 gap-4">
                        {[1, 2, 3].map(i => (
                            <div key={i} className="h-40 bg-white rounded-lg border border-[#E0E5F2] animate-pulse shadow-sm" />
                        ))}
                    </div>
                        ...
                        {Object.entries(groupedTreatments).map(([category, items]) => (
                            <div key={category} className="space-y-4">
                                <div className="flex items-center gap-4 px-4 pt-2">
                                    ...
                                    <span className="text-[10px] font-medium text-[#A3AED0] uppercase tracking-widest opacity-40">{items.length} units</span>
                                </div>
                                        ...
                                        <table className="w-full text-left">
                                            <thead>
                                                <tr className="border-b border-[#F4F7FE]">
                                                    <th className="px-5 py-5 text-[11px] font-medium text-[#A3AED0] uppercase tracking-widest w-[40%]">Procedure Name</th>
                                                    <th className="px-5 py-5 text-[11px] font-medium text-[#A3AED0] uppercase tracking-widest w-[20%]">Duration</th>
                                                    <th className="px-5 py-5 text-[11px] font-medium text-[#A3AED0] uppercase tracking-widest w-[25%]">Price (USD)</th>
                                                    ...
                                                    >
                                                        <td className="px-5 py-4">
                                                            <div className="flex items-center gap-4">
                                                                ...
                                                                />
                                                                <span className="text-[12px] font-medium text-[#1B2559] leading-tight font-kantumruy">{t.name}</span>
                                                            </div>
                                                        </td>
                                                        <td className="px-5 py-4">
                                                            <div className="flex items-center gap-2 text-[#A3AED0]">
                                                                <Clock className="w-3.5 h-3.5" />
                                                                <span className="text-[12px] font-medium text-[#1B2559]">{t.duration_minutes} min</span>
                                                            </div>
                                                        </td>
                                                        <td className="px-5 py-4">
                                                            <span className="text-[12px] font-medium text-[#1B2559] block tracking-tight">${t.price.toLocaleString()}</span>
                                                        </td>
                                                        <td className="px-5 py-4">
                                                            <div className="w-8 h-8 rounded-lg bg-white border border-[#E0E5F2] flex items-center justify-center text-[#A3AED0] hover:bg-[#3B82F6] hover:text-white hover:border-[#3B82F6] opacity-0 group-hover:opacity-100 transition-all shadow-sm">
                    ...
                    <div className="bg-white border-2 border-dashed border-[#E0E5F2] p-24 rounded-[32px] flex flex-col items-center text-center mx-4">
                        ...
                        <h3 className="text-xl font-medium text-[#1B2559] mb-2 tracking-tight">Procedural Gap Identified</h3>
                        <p className="text-xs font-medium text-[#707EAE] max-w-sm">No clinical services found matching your current parameters. Define a new procedure to expand the catalog.</p>
                    </div>
                )}
            </div>
            ...
            {(isAdding || isEditing) && (
                <div className="fixed inset-0 z-[150] flex items-center justify-center bg-[#1B2559]/30 backdrop-blur-md p-4 animate-in fade-in duration-300">
                    <div className="bg-white border border-[#E0E5F2] rounded-lg w-full max-w-md shadow-2xl p-8 space-y-6 relative overflow-hidden">
                        <div className="absolute top-0 left-0 w-full h-1 bg-[#3B82F6]" />

                        <button
                            onClick={() => { setIsAdding(false); setIsEditing(false); }}
                            className="absolute top-6 right-6 p-2 hover:bg-[#F4F7FE] rounded-lg text-[#A3AED0] hover:text-[#3B82F6] transition-all"
                        >
                            <X className="w-5 h-5" />
                        </button>

                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 rounded-lg bg-[#3B82F6]/10 text-[#3B82F6] flex items-center justify-center shadow-inner">
                                {isAdding ? <Plus className="w-6 h-6" /> : <Edit3 className="w-6 h-6" />}
                            </div>
                            <div>
                                <h3 className="text-xl font-medium text-[#1B2559] tracking-tight">{isAdding ? "Create" : "Refine"} Service</h3>
                                <p className="text-[10px] font-medium text-[#A3AED0] uppercase tracking-widest mt-1">Clinical Catalog Entry</p>
                            </div>
                        </div>

                        <div className="space-y-4">
                            <div className="space-y-1.5">
                                <label className="text-[10px] font-medium text-[#A3AED0] uppercase tracking-widest pl-1">Procedure Name</label>
                                <input
                                    type="text"
                                    className="w-full bg-[#F4F7FE] border-none rounded-lg px-4 py-3 text-[12px] font-medium text-[#1B2559] outline-none transition-all placeholder:text-[#94A3B8] focus:ring-2 focus:ring-[#3B82F6]/10"
                            ...
                            <div className="space-y-1.5 relative">
                                <label className="text-[10px] font-medium text-[#A3AED0] uppercase tracking-widest pl-1">Clinical Category</label>
                                <div
                                    className="w-full bg-[#F4F7FE] rounded-lg px-4 py-3 cursor-pointer flex items-center justify-between group"
                                    onClick={() => setIsCategoryDropdownOpen(!isCategoryDropdownOpen)}
                                >
                                    {(isAdding ? newTreatment.category : editingTreatment?.category) ? (
                                        <div
                                            className="px-3 py-1 rounded-lg text-[9px] font-medium uppercase tracking-widest text-white shadow-sm inline-block"
                                            style={{ backgroundColor: getCategoryColor(isAdding ? newTreatment.category : editingTreatment?.category || "") }}
                                        >
                                            {isAdding ? newTreatment.category : editingTreatment?.category}
                                        </div>
                                    ) : (
                                        <span className="text-[10px] font-medium text-[#A3AED0]">Select Category...</span>
                                    )}
                                ...
                                {isCategoryDropdownOpen && (
                                    <>
                                        ...
                                        <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-[#E0E5F2] rounded-lg shadow-xl z-[170] p-3 animate-in fade-in zoom-in-95 duration-200">
                                            <div className="flex flex-wrap gap-2">
                                                {CATEGORIES.map(category => (
                                                    <button
                                                        ...
                                                        className={cn(
                                                            "px-3 py-1.5 rounded-lg text-[9px] font-medium uppercase tracking-widest transition-all border",
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-medium text-[#A3AED0] uppercase tracking-widest pl-1">Valuation (USD)</label>
                                    <div className="relative">
                                        <div className="absolute left-4 top-1/2 -translate-y-1/2 text-[#3B82F6] font-medium opacity-30 text-[12px]">$</div>
                                        <input
                                            type="number"
                                            className="w-full bg-[#F4F7FE] border-none rounded-lg px-8 py-3 text-[14px] font-medium text-[#1B2559] outline-none"
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input
                                            type="number"
                                            className="w-full bg-[#F4F7FE] border-none rounded-lg px-10 py-3 text-[14px] font-medium text-[#1B2559] outline-none"
                        ...
                        <div className="flex gap-3 pt-4">
                            {isEditing && (
                                <button
                                    ...
                                    className="p-3 bg-red-50 text-red-500 hover:bg-red-100 rounded-lg transition-colors shrink-0"
                                >
                            ...
                            <button
                                onClick={() => { setIsAdding(false); setIsEditing(false); }}
                                className="flex-1 bg-[#F4F7FE] hover:bg-[#E9EDF7] text-[#1B2559] py-3 rounded-lg text-[11px] font-medium uppercase tracking-widest transition-all"
                            >
                                Cancel
                            </button>
                            <button
                                ...
                                disabled={submitting}
                                className="flex-[2] bg-[#3B82F6] hover:bg-[#2563EB] text-white py-3 rounded-lg text-[11px] font-medium uppercase tracking-widest shadow-md shadow-[#3B82F6]/20 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                            >
                                {submitting ? (
